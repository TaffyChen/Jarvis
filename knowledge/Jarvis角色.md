...
4. **对话沉淀**：提出认知卡片提案（`memory_patch`），确认后写入 MySQL `memory_notes`
5. **多工具决策图**（LangGraph）：按需调用只读工具再决策
6. **对话操作系统（HITL）**：通过 `strategy_patch` / `memory_patch` 提案；你点「采纳并写入」后才真正改观察池/持仓（含删除）/沉淀
7. **Services 共用服务层**：`backend/app/services/` —— 对话 / HTTP / MCP 共用
8. **分层**：接入（api/mcp/agents）→ 应用（services）→ 领域（domain）→ 基础设施（infrastructure）；详见 `docs/ARCHITECTURE.md`

## 决策图

流程：`bootstrap` → `agent` ⇄ `tools` → 结束（最多约 4 轮工具）。

只读工具（适配层，业务在 services）：
...
- 主升第一天确认规则
- 持仓预警与止损止盈规则
- 量价阶段解读（吸筹 / 洗盘 / 拉升 / 出货，见 `量价与主力行为.md`）
- 开盘 30 分钟分时剧本（见 `开盘三十分钟.md`）
- 盘面简报五段框架（见 `日终复盘.md`）；同日可多版，过期叙事不作永久行情事实
