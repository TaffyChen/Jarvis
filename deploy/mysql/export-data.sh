...
OUT="$OUT_DIR/jarvis-data-$STAMP.sql"
docker exec jarvis-mysql mysqldump \
  -u"${MYSQL_USER:-jarvis}" \
  -p"${MYSQL_PASSWORD}" \
  --skip-extended-insert \
  "${MYSQL_DATABASE:-jarvis}" \
  journal_entries watch_codes positions analyses memory_notes strategy_proposals conversations kv_docs \
  > "$OUT"
echo "已导出: $OUT"
