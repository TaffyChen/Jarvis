      ...
      <table class="screen-table">
        <thead>
          <tr>
            ...
            <th>vs开盘</th><th>委比%</th><th>量比</th><th>PE</th><th>评分</th><th>信号</th><th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="!rows.length">
            <td colspan="12" style="text-align:center;color:var(--muted);padding:24px">点击「扫描刷新」</td>
          </tr>
          <tr v-for="(row, i) in rows" :key="row.code" :class="{ dimmed: row.flags?.buyDiscouraged }">
            ...
            <td :class="chgClass(row.vsOpenPct)">{{ fmtPct(row.vsOpenPct) }}</td>
